...
const RouteCard = ({ route, index, onViewDetails }: RouteCardProps) => {
  ...
  return (
    <motion.div
      ...
      className="glass-card p-6 rounded-[2rem] hover:border-primary/50 transition-all cursor-pointer group relative overflow-hidden shadow-sm"
    >
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="flex-1 space-y-6">
          ...
          <div className="flex items-center gap-8">
            <div className="space-y-1">
              ...
              <p className="text-lg font-bold text-slate-900">{formatDuration(route.totalDuration)}</p>
        ...
        <div className="flex flex-col items-end gap-4">
          ...
          <p className="text-[10px] text-slate-400 font-medium">Compare All Routes →</p>
        </div>
      </div>
    </motion.div>
  );
